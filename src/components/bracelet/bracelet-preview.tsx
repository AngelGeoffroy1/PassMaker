          ...
          <TabsContent value="preview" className="space-y-4">
            <div className="border rounded-lg p-6 bg-gray-50">
              <div className="mb-4 text-center">
                <Badge variant="secondary">Format: Tyvek 150×14mm • 300 DPI</Badge>
              </div>
              
              <div className="relative mx-auto bg-white shadow-lg rounded border" 
                   style={{ 
                     width: '600px', 
                ...
                {/* Overlay pour zones techniques */}
                {(technicalZones.qrCode.enabled || technicalZones.numbering.enabled) && (
                  <div className="absolute inset-0">
                    {technicalZones.qrCode.enabled && (
                      <div className={`absolute top-0 bottom-0 w-8 bg-blue-100 bg-opacity-50 border-2 border-dashed border-blue-300 flex items-center justify-center ${
                        technicalZones.qrCode.position === 'left' ? 'left-0' : 'right-0'
                      }`}>
                        <span className="text-xs font-mono text-blue-600">QR</span>
                      </div>
                    )}
                    
                    {technicalZones.numbering.enabled && (
                      <div className={`absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-green-100 bg-opacity-50 border border-dashed border-green-300 flex items-center justify-center ${
                        technicalZones.numbering.position === 'left' ? 'left-0' : 'right-0'
                      } ${technicalZones.qrCode.enabled && technicalZones.qrCode.position === technicalZones.numbering.position ? 'ml-8' : ''}`}>
                        <span className="text-[8px] font-mono text-green-600">#1</span>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between py-2">
                    ...
                    <span>{technicalZones.cutMarks ? 'Repères inclus' : 'Aucun repère'}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between py-2 border-b">
                    ...
                    <span className="flex items-center gap-2">
                      {technicalZones.qrCode.enabled ? (
                        <>
                          <Badge variant="outline" className="bg-green-50">20×20mm</Badge>
                  ...
                  <div className="flex justify-between py-2 border-b">
                    ...
                    <span className="flex items-center gap-2">
                      {technicalZones.numbering.enabled ? (
                        <>
                          <Badge variant="outline" className="bg-green-50">10×5mm</Badge>
                          <span className="text-xs">({technicalZones.numbering.position})</span>
                        </>
                      ) : (
                        <Badge variant="secondary">Désactivé</Badge>
                      )}
